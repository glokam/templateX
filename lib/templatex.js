...
function _render(template) {
    ...
    if (indexStart === -1 || indexEnd === -1) return template.replace(/\%\{/gi, '{');
...
function _iterate(opt) {
    ...
    if (indexEnd === -1)  new Error("Missing ending tag for {{" + opt.substring + "}}!!");
...
module.exports = {
    render: function (template, dataObj) {
        data = dataObj;
    ...
    express:  function (filePath, options, callback) {
        fs.readFile(filePath, function (err, content) {
            ...
            var rendered = module.exports.render(content.toString(), options)
